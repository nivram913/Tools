...
table = dynamodb.Table('op_noble_cause_cache')
CACHE_KEY = "teamup_cache"
CACHE_TTL = 60*60  # cache 60 minutes
...
def lambda_handler(event, context):
    try:
        cache_item = table.get_item(Key={"id": CACHE_KEY}).get("Item")
        if cache_item and (int(time.time()) - cache_item["timestamp"]) < CACHE_TTL:
            returned_html = cache_item["data"]
        else:
            # --- Fetch Teamup ---
            ...
            <body>
                ...
                <h4 style="color: red">État à {montreal_time.strftime("%Y-%m-%d %H:%M:%S")} (Version en cache)</h4>

                <div class="tabs">
                    <div id="tab-ymca" class="tab active" onclick="showTab('ymca')">YMCA Centre-ville</div>
                    <div id="tab-hd" class="tab" onclick="showTab('hd')">Hôtel Dieu</div>
                </div>

                <div id="content-hd" class="tab-content">
                    <h2>Hôtel Dieu</h2>
                ...
                <div id="content-ymca" class="tab-content active">
